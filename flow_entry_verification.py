...
class flow_entry_verification(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.switch_info = {}
        self.graph = {}
        for i in xrange(1,topo_gen.SWITCH_NUM+1):
            self.graph[i] = set()
    ...
    def get_dst_count(self,dst_switch):
        ret = {}
        for i in xrange(1,topo_gen.SWITCH_NUM+1):
            ret[i] = 0
        for switch in self.graph:
            for entry in self.graph[switch]:
                on_switch,match_field,value,out_port = entry
                print entry
                print entry,'    ' ,dst_switch[on_switch][out_port-1]
                #ret[topo_gen.dst_switch[on_switch][out_port-1]] += 1
        return ret

    def aggregate_groups(self):
        print topo_gen.dst_switch
        f = open('port_to_switch.txt','r')
        dst_switch = eval(f.read())
        f.close()
        switch_as_dst = self.get_dst_count(dst_switch)
        #print switch_as_dst
    ...
    def switch_features_handler(self, ev):
        ...
        if self.switch_counter == topo_gen.SWITCH_NUM:    #all switch ready
